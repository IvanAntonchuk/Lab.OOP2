# Проєкт: Link Manager (Менеджер веброзширень)

## Лабораторна робота №1a (II семестр) — Рефакторинг та моделювання з UML

**Мета:** Глибокий рефакторинг монолітного застосунку, впровадження компонентної архітектури (Library), застосування SOLID-принципів та повне візуальне моделювання системи.

### Архітектурні зміни та рефакторинг
Проєкт було повністю переосмислено та переведено на сучасну багатокомпонентну архітектуру:

1.  **Компонентне розділення (Core Library):**
    * Всю бізнес-логіку винесено в окрему статичну бібліотеку **`LinkManagerCore`**. 
    * Це забезпечує принцип **Low Coupling** (низької зв'язності) — логіка роботи з даними тепер повністю відокремлена від графічного інтерфейсу (UI).
2.  **Впровадження MVC (Model-View-Controller):**
    * **Model:** Класи `LinkManager` та `LinkData`.
    * **View:** `MainWindow` та набір діалогових вікон.
    * **Controller:** `MainController`, який координує взаємодію між UI та ядром системи.
3.  **Патерни проєктування:**
    * **Strategy (Стратегія):** Використовується для експорту/імпорту. Впроваджено інтерфейси `IExportStrategy` та `IImportStrategy`. Це дозволяє додавати нові формати (XML, PDF) без зміни існуючого коду (**Open/Closed Principle**).
    * **Singleton (Одинак):** Клас `DatabaseManager` реалізований як Singleton для гарантування єдиної точки доступу до БД SQLite.
4.  **Асинхронна мережева логіка:**
    * Сторонню бібліотеку `httplib` замінено на нативний Qt клас `QNetworkAccessManager` (клас `WebUtils`). Це забезпечило **неблокуюче** завантаження даних.

### Тестування та аналіз метрик (Бенчмарки)
Для підтвердження ефективності рефакторингу проведено заміри продуктивності до та після змін:

| Метрика / Характеристика | До рефакторингу (Моноліт + `httplib`) | Після рефакторингу (MVC + `LinkManagerCore`) | Оцінка покращення |
| :--- | :--- | :--- | :--- |
| **Характер мережевих запитів** | Синхронні/Блокуючі (зависання UI на 1-2 сек) | Асинхронні (`QNetworkAccessManager`) | **Повне усунення фризів UI** |
| **Розмір сторонніх залежностей** | Наявність важкого заголовка `httplib.h` | Використання нативних модулів `Qt Network` | **Зменшення обсягу коду** |
| **Споживання пам'яті (RAM)** | ~45 MB (додаткові потоки httplib) | ~38 MB (оптимізований event-loop Qt) | **Зменшення на ~15%** |
| **Швидкість завантаження заголовків** | Залежить від тайм-аутів потоків | Оптимальна через чергу запитів Qt | Стабільна робота під навантаженням |
| **Архітектурна зв'язність (Coupling)**| Висока (High) – логіка вшита у віджети | Низька (Low) – незалежна бібліотека `.a` / `.lib` | **Відповідність SOLID (O, S)** |

### UML Моделювання
Проєкт супроводжується повним набором UML-діаграм, що описують систему з різних ракурсів (структура, поведінка, взаємодія):
* **Structure:** Class, Component, Deployment, Object, Package, Composite Structure.
* **Behavior:** Use Case, Activity, State Machine.
* **Interaction:** Sequence, Communication, Interaction Overview, Timing.
* **Глосарій:** Створено детальний технічний глосарій предметної галузі (знаходиться в папці `UML_Models`).

### Документація
Повна технічна документація згенерована за допомогою **Doxygen**. Налаштовано автоматичну генерацію UML-діаграм залежностей класів (Graphviz).
🔗 **Документація онлайн:** [https://ivanantonchuk.github.io/Lab.OOP2/]
